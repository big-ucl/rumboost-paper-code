...
from biogeme.models import loglogit, logit, lognested, nested, logcnl, cnl_avail
...
def SwissMetro_normalised(dataset_train: pd.DataFrame, for_prob=False):
    ...
    V_TRAIN = ASC_TRAIN + B_TIME * TRAIN_TT + B_COST * TRAIN_COST + B_HE * TRAIN_HE + B_FIRST * FIRST + B_MALE * MALE + B_PURPOSE_1 * PURPOSE_1 + B_PURPOSE_2 * PURPOSE_2 + B_PURPOSE_3 * PURPOSE_3 + B_PURPOSE_4 * PURPOSE_4 + B_PURPOSE_5 * PURPOSE_5 + B_PURPOSE_6 * PURPOSE_6 + B_PURPOSE_7 * PURPOSE_7 + B_PURPOSE_8 * PURPOSE_8 + B_AGE_1 * AGE_1 + B_AGE_2 * AGE_2 #+ B_AGE_3 * AGE_3 + B_AGE_4 * AGE_4 #+ B_ORIG_TIC * ORIG_TIC + B_DEST_TIC * DEST_TIC + B_SEV_LUGGAGES * SEV_LUGGAGES + B_ORIG_ROM * ORIG_ROM + B_DEST_ROM * DEST_ROM 
    V_SM    = ASC_SM    + B_TIME * SM_TT    + B_COST * SM_COST    + B_HE * SM_HE    + B_MALE * MALE + B_PURPOSE_1 * PURPOSE_1 + B_PURPOSE_2 * PURPOSE_2 + B_PURPOSE_3 * PURPOSE_3 + B_PURPOSE_4 * PURPOSE_4 + B_PURPOSE_5 * PURPOSE_5 + B_PURPOSE_6 * PURPOSE_6 + B_PURPOSE_7 * PURPOSE_7 + B_PURPOSE_8 * PURPOSE_8 + B_AGE_1 * AGE_1 + B_AGE_2 * AGE_2 #+ B_AGE_3 * AGE_3 + B_AGE_4 * AGE_4 #+ B_ORIG_TIC * ORIG_TIC + B_DEST_TIC * DEST_TIC + B_SEV_LUGGAGES * SEV_LUGGAGES + B_SM_SEATS * SM_SEATS + B_ORIG_ROM * ORIG_ROM + B_DEST_ROM * DEST_ROM 
    V_CAR   = ASC_CAR   + B_TIME * CAR_TT   + B_COST * CAR_CO #+ B_SEV_LUGGAGES * SEV_LUGGAGES #+ B_MALE * MALE   + B_SEV_LUGGAGES * SEV_LUGGAGES + B_ORIG_ROM * ORIG_ROM + B_ORIG_TIC * ORIG_TIC + B_DEST_ROM * DEST_ROM + B_DEST_TIC * DEST_TIC
...
def SwissMetro_synthetic(dataset_train: pd.DataFrame, for_prob=False):
    '''
    Create a MNL on the swissmetro semi-syhtetic dataset.
    ...
    B_COST_CAR  = Beta('B_COST_CAR', 0, None, 0, 0)
    B_TIME_RAIL = Beta('B_TIME_RAIL', 0, None, 0, 0)
    B_COST_HE_RAIL = Beta('B_COST_HE_RAIL', 0, None, 0, 0)
    B_TIME_SM   = Beta('B_TIME_SM', 0, None, 0, 0)
    B_COST_HE_SM   = Beta('B_COST_SM', 0, None, 0, 0)

    SM_COST_HE = SM_COST * SM_HE
    TRAIN_COST_HE = TRAIN_COST * TRAIN_HE



    #utilities
    V_TRAIN = ASC_TRAIN + B_TIME_RAIL * TRAIN_TT + B_COST_HE_RAIL * TRAIN_COST_HE
    V_SM    = ASC_SM    + B_TIME_SM * SM_TT    + B_COST_HE_SM * SM_COST_HE
    V_CAR   = ASC_CAR   + B_TIME_CAR * CAR_TT   + B_COST_CAR * CAR_CO 
...
def LPMC_mixed_logit_tt(dataset_train: pd.DataFrame, for_prob=False):
    '''
    Create a mixture of logit on the lpmc dataset.
    ...
    for beta in MNL_beta_params_s:
        exec("{} = Beta('{}', 1, None, None, 0)".format(beta, beta), globals())
    


    exec("B_dur_walking_Walk_Random = B_dur_walking_Walk + B_dur_walking_S * bioDraws('B_NORMAL', 'NORMAL')", globals())
    exec("B_dur_cycling_Bike_Random = B_dur_cycling_Bike + B_dur_cycling_S * bioDraws('B_NORMAL', 'NORMAL')", globals())
    exec("B_dur_pt_rail_Public_Transport_Random = B_dur_pt_rail_Public_Transport + B_dur_pt_rail_S * bioDraws('B_NORMAL', 'NORMAL')", globals())
    exec("B_dur_driving_Car_Random = B_dur_driving_Car + B_dur_driving_S * bioDraws('B_NORMAL', 'NORMAL')", globals())
    ...
    biogeme = bio.BIOGEME(database_train, logprob)
    #biogeme.number_of_draws = 50
    biogeme.modelName = 'LPMC_mixed_logit'
    ...
    if for_prob:
        prob_0 = MonteCarlo(logit(V_dict, av, 0))
        prob_1 = MonteCarlo(logit(V_dict, av, 1))
        prob_2 = MonteCarlo(logit(V_dict, av, 2))
        prob_3 = MonteCarlo(logit(V_dict, av, 3))

        simulate = {'Prob. 0': prob_0, 'Prob. 1': prob_1, 'Prob. 2': prob_2, 'Prob. 3': prob_3}
        biosim = bio.BIOGEME(database_train, simulate, number_of_draws=number_of_draws)
        biosim.modelName = "LPMC_mixed_logit_test"
        biosim.number_of_draws = 50
...
def LPMC_synthetic(dataset_train: pd.DataFrame, for_prob=False):
    '''
    Create a MNL on the lpmc semi-syhtetic dataset.
    ...
    exec("dur_pt = dur_pt_access + dur_pt_rail + dur_pt_bus + dur_pt_int_waiting + dur_pt_int_walking", globals())
    exec("age_dur_cycling = age*dur_cycling", globals())

    #driving_percentage, congestion charge as a binary variable NEW PB 0.6730 with lr = 0.1
    MNL_beta_params_negative = ['B_dur_walking_Walk',  'B_dur_cycling_age_Bike', 'B_dur_pt_Public_Transport','B_dur_driving_Car', 'B_distance']
    MNL_beta_params_neutral = [ 'ASC_Walk', 'ASC_Bike', 'ASC_Public_Transport'] 

    MNL_utilities = {0: 'ASC_Walk + B_dur_walking_Walk*dur_walking',
                        1: 'ASC_Bike + B_dur_cycling_age_Bike*age_dur_cycling',
                        2: 'ASC_Public_Transport + B_dur_pt_Public_Transport*dur_pt',
                        3: 'B_dur_driving_Car*dur_driving + B_distance*distance'}
    ...
    if for_prob:
        prob_train = logit(V_dict, av, 0)
        prob_SM = logit(V_dict, av, 1)
        prob_car = logit(V_dict, av, 2)
...
def LPMC_cross_nested_normalised(dataset_train, for_prob=False):
    '''
    Create a MNL on the LPMC dataset.
    The model is a slightly modified version from teh code that can be found here: https://github.com/JoseAngelMartinB/prediction-behavioural-analysis-ml-travel-mode-choice.
    ...
    #MU_nm = Beta('MU_nm', 1, 1, 10, 0)
    MU_m = Beta('MU_m', 1, 1, 10, 0)
    MU_f = Beta('MU_f', 1, 1, 10, 0)
    alpha_f = Beta('alpha_f',0.5,0,1,0)
    alpha_m = 1-alpha_f

    #Definition of nests
    # Nest membership parameters
    alpha_flexible = {0: 1.0,
                    1:1.0,
                    2:0.0,
                    3:alpha_f}

    alpha_motorised = {0: 0.0,
                    1:0.0,
                    2:1.0,
                    3:alpha_m}

    nest_flexible = MU_f, alpha_flexible
    nest_motorised = MU_m, alpha_motorised
    nests = nest_flexible, nest_motorised

    #definition of the model
    logprob = logcnl(V_dict, av, nests, choice)

    #create the Biogeme object
    biogeme = bio.BIOGEME(database_train, logprob)
    biogeme.modelName = 'LPMC_CNL'

    biogeme.generate_html = True
    biogeme.generate_pickle = True

    if for_prob:
        prob_0 = cnl_avail(V_dict, av, nests, 0)
        prob_1 = cnl_avail(V_dict, av, nests, 1)
        prob_2 = cnl_avail(V_dict, av, nests, 2)
        prob_3 = cnl_avail(V_dict, av, nests, 3)

        simulate = {'Prob. 0': prob_0, 'Prob. 1': prob_1, 'Prob. 2': prob_2, 'Prob. 3': prob_3}
        biosim = bio.BIOGEME(database_train, simulate)
        biosim.modelName = "LPMC_CNlogit_test"
